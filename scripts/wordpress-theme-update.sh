...
TYPE="theme" # Set the update type (plugin, theme, core)
...
UPDATED_THEMES_LINES=$(echo "$UPDATE_OUTPUT" | grep -E '^[[:space:]]*\|.*\|.*\|.*\|[[:space:]]*Updated[[:space:]]*\|$')

if [ -n "$UPDATED_THEMES_LINES" ]; then
    # Use awk to extract name (field 2) and new_version (field 4)
    # Format as: Name (Version)
    THEME_DETAILS=$(printf "%s" "$UPDATED_THEMES_LINES" | awk -F'|' '{
        gsub(/^[ \t]+|[ \t]+$/, "", $2); # Trim name
